<template>
  <v-row justify="center" align="center" class="pa-4">
    <v-col cols="12" md="4">
      <h1 class="display-3" align="center">Регистрация</h1>
      <div v-if="isLoading">
        <br>
        <v-progress-linear
          indeterminate
      ...
      <v-form v-model="valid">
        ...
        <v-text-field
          ...
          required
          :error="'invite' in errors"
          :error-messages="errors.invite"
          hide-details="auto"
        ></v-text-field>
        <v-text-field
          :disabled="isLoading"
        ...
        <v-text-field
          ...
          :error-messages="errors.password_repeat"
          hide-details="auto"
        ></v-text-field>
        <v-btn
          :disabled="isLoading"
      ...
      <v-btn text small block :to="{ name: 'Login' }" color="primary">Уже зарегистрированы?</v-btn>
    </v-col>
  </v-row>
</template>
...
export default {
  ...
  mounted() {
    eventBus.$emit('setTitle', 'Регистрация')
  },
  methods: {
    validate(e) {
      e.preventDefault()
